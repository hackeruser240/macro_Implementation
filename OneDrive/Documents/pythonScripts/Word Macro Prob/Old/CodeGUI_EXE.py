...
def create_pdf_folder(base_path, folder_name="PDFs"):
    ...
    log_message("=" * 10)
    log_message(f"PDF folder created: {folder_path}") # Modified print to log_message
    return folder_path
...
def convert_docm_to_pdf(input_path, output_path):
    ...
    try:
        ...
        log_message("PDF created!") # Modified print to log_message
...
def apply_formatting(index, filepath):
    ...
    #log_message(f"location of word file: {word_file}") # Modified print to log_message   
...
def start_conversion():
    ...
    for file in filenames:
        log_message("=" * 10)
        log_message(f"Creating PDF of {file}")
    ...
    log_message(f"All {len(filenames)} successfully processed!")
